...
with tf.name_scope('layer1') as scope: #input layer
    W1 = tf.get_variable('weight1', shape = [2, 10], initializer= tf.contrib.layers.xavier_initializer())
...
with tf.name_scope('layer5') as scope:
    W5 = tf.get_variable('weight5', shape = [10, 10], initializer= tf.contrib.layers.xavier_initializer())
    b5 = tf.Variable(tf.random_uniform([10], -1.0, 1.0, name = 'bias5'))
    layer5 = tf.nn.relu(tf.matmul(layer4, W5) + b5)    

with tf.name_scope('layer6') as scope:
    W6 = tf.get_variable('weight6', shape = [10, 10], initializer= tf.contrib.layers.xavier_initializer())
    b6 = tf.Variable(tf.random_uniform([10], -1.0, 1.0, name = 'bias6'))
    layer6 = tf.nn.relu(tf.matmul(layer5, W6) + b6)    

with tf.name_scope('layer7') as scope:
    W7 = tf.get_variable('weight7', shape = [10, 10], initializer= tf.contrib.layers.xavier_initializer())
    b7 = tf.Variable(tf.random_uniform([10], -1.0, 1.0, name = 'bias7'))
    layer7 = tf.nn.relu(tf.matmul(layer6, W7) + b7)    

with tf.name_scope('layer8') as scope:
    W8 = tf.get_variable('weight8', shape = [10, 10], initializer= tf.contrib.layers.xavier_initializer())
    b8 = tf.Variable(tf.random_uniform([10], -1.0, 1.0, name = 'bias8'))
    layer8 = tf.nn.relu(tf.matmul(layer7, W8) + b8)    

with tf.name_scope('layer9') as scope:
    W9 = tf.get_variable('weight9', shape = [10, 10], initializer= tf.contrib.layers.xavier_initializer())
...
with tf.name_scope('layer10') as scope: #output layer
    W10 = tf.get_variable('weight10', shape = [10, 1], initializer= tf.contrib.layers.xavier_initializer())
...
writer = tf.summary.FileWriter('./compare/relu_xavier')
